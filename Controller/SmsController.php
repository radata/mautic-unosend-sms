...
namespace MauticPlugin\UnosendSmsBundle\Controller;

use Mautic\CoreBundle\Controller\FormController;
use Mautic\SmsBundle\Sms\TransportChain;
use MauticPlugin\UnosendSmsBundle\Form\Type\SendSmsType;
...
class SmsController extends FormController
{
    public function sendSmsAction(Request $request, TransportChain $transportChain, $objectId = ''): JsonResponse|Response
    {
        if ('POST' === $request->getMethod()) {
            $data     = $request->request->all()['unosend_send_sms'] ?? [];
        ...
        if ('GET' === $request->getMethod()) {
            ...
            return $this->delegateView([
                ...
                'contentTemplate' => '@UnosendSms/SendSms/form.html.twig',
                'passthroughVars' => [
        ...
        if ('POST' === $request->getMethod()) {
            ...
            if (!empty($smsId)) {
                ...
                if (!$sms || !$sms->isPublished()) {
                    $this->addFlashMessage('unosend_sms.send.error.not_found', [], 'error');
            ...
            if (empty($content)) {
                $this->addFlashMessage('unosend_sms.send.error.no_content', [], 'error');
            ...
            try {
                ...
                $this->addFlashMessage('unosend_sms.send.error.transport', [], 'error');
            ...
            if (true === $result) {
                $this->addFlashMessage('unosend_sms.send.success');
            } else {
                $this->addFlashMessage('unosend_sms.send.error.failed_detail', ['%error%' => $result], 'error');
            }
